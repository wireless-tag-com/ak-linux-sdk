...
install()
{
    echo "install sc3335 driver"
#    v4l2_install
    /sbin/insmod /usr/modules/ak_isp.ko
    /sbin/insmod /usr/modules/sensor_sc3335.ko
    if test ! -f /etc/config/isp_sc3335_mipi_2lane_h3b.conf ; then
        rm /etc/config/isp_*.conf
        cp /etc/isp_sc3335_mipi_2lane_h3b.conf /etc/config/
    fi
}

uninstall()
{
    echo "uninstall sc3335 driver"
    /sbin/rmmod ak_isp.ko
    /sbin/rmmod sensor_sc3335.ko
#    v4l2_uninstall
}
