...
install()
{
    echo "install sc2335 driver"
#    v4l2_install
    /sbin/insmod /usr/modules/ak_isp.ko
    /sbin/insmod /usr/modules/sensor_sc2335.ko
    if test ! -f /etc/config/isp_sc2335_mipi_2lane.conf ; then
        rm /etc/config/isp_*.conf
        cp /etc/isp_sc2335_mipi_2lane.conf /etc/config/
    fi
}

uninstall()
{
    echo "uninstall sc2335 driver"
    /sbin/rmmod ak_isp.ko
    /sbin/rmmod sensor_sc2335.ko
#    v4l2_uninstall
}
