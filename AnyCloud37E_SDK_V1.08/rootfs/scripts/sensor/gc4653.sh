...
#    modprobe videobuf2_v4l2.ko
    ...
    insmod /usr/modules/ak_isp.ko internal_pclk=100000000
    insmod /usr/modules/sensor_gc4653.ko

    if test ! -f /etc/config/isp_gc4653_mipi_2lane_h3b_20fps.conf ; then
        rm /etc/config/isp_*.conf
        cp /etc/isp_gc4653_mipi_2lane_h3b_20fps.conf /etc/config/
    fi
...
uninstall()
{
    rmmod /usr/modules/sensor_gc4653.ko
...
{
    echo "usage :" 
    echo "    ./gc4653.sh <install/uninstall>"
