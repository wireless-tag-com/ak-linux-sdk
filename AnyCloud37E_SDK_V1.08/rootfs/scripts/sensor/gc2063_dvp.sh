...
#    modprobe videobuf2_v4l2.ko
    ...
    insmod /usr/modules/sensor_gc2063.ko dvp=1 check_id=1 SENSOR_I2C_ADDR=0x37

    if test ! -f /etc/config/isp_gc2063_dvp.conf ; then
        rm /etc/config/isp_*.conf
        cp /etc/isp_gc2063_dvp.conf /etc/config/
    fi
