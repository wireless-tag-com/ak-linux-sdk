...
install()
{
    echo "install ps5268 driver"
#    v4l2_install
    /sbin/insmod /usr/modules/ak_isp.ko
    /sbin/insmod /usr/modules/sensor_ps5268.ko
#    if test ! -f /etc/config/isp_ps5268_mipi_2lane_h3b.conf ; then
    if test ! -f /etc/config/isp_ps5268_mipi_2lane_h3b.conf ; then
        rm /etc/config/isp_*.conf
#        cp /etc/isp_ps5268_mipi_2lane_h3b.conf /etc/config/
        cp /etc/isp_ps5268_mipi_2lane_h3b.conf /etc/config/
    fi
}
